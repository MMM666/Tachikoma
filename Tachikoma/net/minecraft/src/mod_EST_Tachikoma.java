...
public class mod_EST_Tachikoma extends BaseMod {
	...
	public void addRenderer(Map map) {
		try {
			if (isSpiderForm) {
				map.put(net.minecraft.src.EST_EntityTachikoma.class,
						new EST_RenderTachikoma());
				
				for (int i = 0; i < 16; i++) {
					texturelist
							.add(String
									.format("/mob/Tachikoma/CV_Tachikoma/Tachikoma_%02x.png",
											i));
				}
				texturelist.add("/mob/Tachikoma/CV_Tachikoma/Tachikoma_3b.png");
				for (int i = 0; i < 16 && isSpider_OptCam; i++) {
					texturelist
							.add(String
									.format("/mob/Tachikoma/OptCam_Tachikoma/Tachikoma_%02x.png",
											i));
				}
			}
			
			if (isPlayerForm) {
				map.put(net.minecraft.src.EntityPlayerSP.class,
						new EST_RenderPlayerFormTachikoma());
